...
if not ok then
  print("Script creation failed: " .. tostring(err))
  return
end

if not file_exists(script_path) then
  print("File '" .. script_path .. "' was not created.")
  return
end

local chmod_status = cmd_run("chmod +x " .. script_path)
if not chmod_status then
  print("Failed to make script executable.")
  return
end

local exec_status = cmd_run("./" .. script_path)
