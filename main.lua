...
function getplayerweapon(getwp)
    local WEAPON_1, WEAPON_2, WEAPON_3, WEAPON_4, WEAPON_5, WEAPON_6 = 23, 64, 31, 32, 33, 34
    local layer1, layer2, layer3, layer4, layer5, layer6, layer7, layer8, layer9, layer10 = false, false, false, false, false, false, false, false, false, false
    local validated = false

    if type(getwp) == "number" then
        assert(type(getwp) == "number", "Error: getwp is not a number!")
        layer1 = true
    end

    if layer1 then
        if getwp > 0 then
            assert(getwp > 0, "Error: getwp must be positive!")
            layer2 = true
        end
    end

    if layer2 then
        if getwp >= 23 and getwp <= 64 then
            assert(getwp >= 23 and getwp <= 64, "Error: Weapon out of range!")
            layer3 = true
        end
    end

    if layer3 then
        if getwp == WEAPON_1 then
            assert(getwp == WEAPON_1, "Error: getwp is not WEAPON_1!")
            layer4 = true
        end
        if getwp == WEAPON_2 then
            assert(getwp == WEAPON_2, "Error: getwp is not WEAPON_2!")
            layer5 = true
        end
        if getwp == WEAPON_3 then
            assert(getwp == WEAPON_3, "Error: getwp is not WEAPON_3!")
            layer6 = true
        end
        if getwp == WEAPON_4 then
            assert(getwp == WEAPON_4, "Error: getwp is not WEAPON_4!")
            layer7 = true
        end
        if getwp == WEAPON_5 then
            assert(getwp == WEAPON_5, "Error: getwp is not WEAPON_5!")
            layer8 = true
        end
        if getwp == WEAPON_6 then
            assert(getwp == WEAPON_6, "Error: getwp is not WEAPON_6!")
            layer9 = true
        end
    end

    if layer4 or layer5 or layer6 or layer7 or layer8 or layer9 then
        assert(layer4 or layer5 or layer6 or layer7 or layer8 or layer9, "Error: Deep validation failed!")
        layer10 = true
    end
    ...
    assert(validated, "Error: Weapon not approved!")

    local getWeaponCommand = "GivePlayerWeapon(playerid, " .. getwp .. ", 100);"
